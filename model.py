...
class Time_Module(nn.Module):
    def __init__(self):
        ...
        # 定义 Transformer 编码器层 Define Transformer encoder layer
...
class MultiScaleDilatedConv(nn.Module):
    def __init__(self):
        ...
        # 定义多个不同膨胀率的卷积层  Convolutional layers with different dilation rates
    ...
    def forward(self, x):
        ...
        x2 = self.path2(x)
        x3 = self.path3(x)
        x2 = F.interpolate(x2, x1.size(2))  # 插值，上采样   # Interpolation, upsampling
...
def normalize_A(A, symmetry=False):
    ...
    if symmetry:
        ...
        d = torch.sum(A, 1)   # 对A的第1维度求和  # Sum up the first dimension of A
...
class ChnGNN_Module(nn.Module):
    def __init__(self, xdim, k_adj, num_out):
        ...
        self.BN1 = nn.BatchNorm1d(xdim[2])  # 对第二维（第一维为batch_size)进行标准化  # Standardize the second dimension
...
class AttentionFusion(nn.Module):
    def __init__(self, input_dim):
        ...
        self.attention_fc = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Linear(64, 1)  # 输出一个权重分数   # Output a weight score
        )
    ...
    def forward(self, feat1, feat2, feat3):
        ...
        # 拼接 & 归一化(softmax)    # Splicing&Normalization (softmax)
        scores = torch.cat([score1, score2, score3], dim=1)  # [B, 3]
        weights = F.softmax(scores, dim=1)  # [B, 3]

        # 展开权重维度以便逐模态乘法    # Expand the weight dimension for modal multiplication
...
class Mymodel(nn.Module):
    ...
    def forward(self, time_feat, image_feat):
        ...
        # # 融合方式一_神经网络融合     # Method 1_AttentionFusion
        # fused_feature, attn_weights = self.fusion_module(rgb_out, time_out, gnn_out)

        # 融合方式二_点乘融合       # Method 2_multiplication fusion
