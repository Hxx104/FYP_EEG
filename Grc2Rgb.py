...
def split_into_rgb(data):
    ...
    # 计算每一组的实际大小  # Calculate the actual size of each group
    sizes = [group_size + (1 if i < remainder else 0) for i in range(3)]

    # 得到切分索引   # Obtain split index
...
def Gr2Rgb(time_data, phase_data, img_size=224):
    ...
    for trial in range(trials):
        for w in range(windows):
            ...
            # 只画边框四条线    # Only four lines around the border
            output[trial, y1_img, x1:x2 + 1] = 1  # 上边
            output[trial, y2_img, x1:x2 + 1] = 1  # 下边
            output[trial, y1_img:y2_img + 1, x1] = 1  # 左边
            output[trial, y1_img:y2_img + 1, x2] = 1  # 右边
